    ...
    <article className={`relative group flex h-full flex-col bg-[#0a0a0a] border border-white/5 rounded-2xl overflow-hidden transition-all duration-300 hover:border-[var(--accent)] hover:shadow-[0_0_30px_rgba(99,102,241,0.1)] ${compact ? "min-h-0" : ""}`}>
      ...
      <div className={`relative w-full overflow-hidden ${compact ? "h-40" : "h-64"}`}>
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          <span className="px-2 py-1 rounded-md bg-black/60 text-[9px] font-black text-white uppercase tracking-widest backdrop-blur-md border border-white/10">
      ...
      <div className={`flex flex-1 flex-col gap-3 p-4 md:p-5`}>
        <div className="space-y-1">
          <h3 className={`font-black text-white line-clamp-1 leading-tight tracking-tight uppercase text-xs md:text-sm`}>
            {ebook.title}
          </h3>
          {!compact && (
        ...
        <div className="mt-auto flex items-center justify-between pt-2">
          ...
          <Link 
            href={`/ebook/${ebook.id}`} 
            onClick={onViewClick}
            className="px-4 py-2 bg-white/5 text-[var(--accent)] text-[10px] font-black rounded-full uppercase tracking-widest hover:bg-[var(--accent)] hover:text-white transition-all"
          >
            View
          </Link>
        </div>
